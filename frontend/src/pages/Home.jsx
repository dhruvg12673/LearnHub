...
import { getUserRoadmaps, generateRoadmap, getRoadmap } from '../api/roadmap';
...
const Home = () => {
  ...
  const fetchRoadmaps = async () => {
    try {
      ...
      // For each roadmap get detailed roadmap and compute progress
      const progressForAll = {};
      await Promise.all(data.map(async (rm) => {
        try {
          const full = await getRoadmap(rm.id);
          const { total, completed } = countNodesCompleted(full);
          const percent = total === 0 ? 0 : Math.round((completed / total) * 100);
          progressForAll[rm.id] = percent;
        } catch (e) {
          progressForAll[rm.id] = 0;
        }
      }));
      setProgressMap(progressForAll);
  ...
  const countNodesCompleted = (roadmapFull) => {
    // Walk the roadmap structure to count total nodes and completed nodes (status === 'completed' or 'expert')
    let total = 0;
    let completed = 0;
  ...
  const calculateProgress = (roadmap) => {
    return progressMap[roadmap.id] || 0;
  };
