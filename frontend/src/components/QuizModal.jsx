...
import './QuizModal.css';
  ...
  const QuizModal = ({ isOpen, onClose, topic, subtopic, difficulty, language, roadmapId, onQuizComplete }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (lastQuestionTime) => {
    ...
    try {
      ...
      setResult(data);
      // Notify parent immediately if score >= 60 so UI can update faster
      if (onQuizComplete && data && data.score >= 60) {
        try { onQuizComplete(data, subtopic); } catch (e) { /* ignore */ }
      }
    } catch (error) {
            ...
            <div className="result-view">
              ...
              <div className="result-actions">
                ...
                {result.score < 60 ? (
                  <button 
                    onClick={() => {
                      // Allow retry by resetting state and reloading quiz
                      setResult(null);
                      setQuestions([]);
                      setCurrentQuestionIndex(0);
                      setAnswers({});
                ...
                <button 
                  onClick={() => {
                    // Notify parent of the result so roadmap can refresh/mark completion
                    if (onQuizComplete) onQuizComplete(result, subtopic);
                    onClose();
                  }}
                  className="result-close-button"
                >
