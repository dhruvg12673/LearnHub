...
async def update_knowledge_state(user_id: int, topic: str, subtopic: str, score: int, time_taken: int):
    ...
    if row:
        # If updated, return the updated mastery and its computed status
        return {"mastery": int((row["mastery_score"] * 0.7) + (new_mastery * 0.3)), "status": new_status}
    else:
        return {"mastery": new_mastery, "status": status}
